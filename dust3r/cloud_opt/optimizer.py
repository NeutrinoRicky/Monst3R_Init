...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=False, focal_break=20, shared_focal=False, flow_loss_fn='smooth_l1', flow_loss_weight=0.0, 
        ...
        self.camera_pose_prior_weight = float(camera_pose_prior_weight)
        self.camera_intrinsics_prior_weight = float(camera_intrinsics_prior_weight)
        self.camera_pose_prior_translation_weight = float(camera_pose_prior_translation_weight)
        self.camera_intrinsics_pp_prior_weight = float(camera_intrinsics_pp_prior_weight)
        ...
        self.register_buffer('_camera_prior_image_diag', torch.tensor(
            [np.sqrt(float(h * h + w * w)) for h, w in self.imshapes],
            dtype=torch.float32,
        ))
    ...
    def _setup_camera_prior_targets(self):
        has_pose_target = (
            self.camera_poses is not None
            and isinstance(self.camera_poses, (list, tuple))
            and len(self.camera_poses) == self.n_imgs
        )
        if has_pose_target:
            pose_target = torch.stack([pose.float() for pose in self.camera_poses], dim=0)
            centers = pose_target[:, :3, 3]
            trans_scale = torch.tensor(1.0, dtype=torch.float32)
            if centers.shape[0] >= 2:
                step = torch.linalg.norm(centers[1:] - centers[:-1], dim=1)
                valid = step[torch.isfinite(step) & (step > 1e-8)]
                if valid.numel() > 0:
                    trans_scale = torch.median(valid).float()
            if not torch.isfinite(trans_scale) or float(trans_scale) <= 1e-8:
                trans_scale = torch.tensor(1.0, dtype=torch.float32)
        else:
            pose_target = torch.zeros((0, 4, 4), dtype=torch.float32)
            trans_scale = torch.tensor(1.0, dtype=torch.float32)
        self.register_buffer('_camera_pose_target', pose_target)
        self.register_buffer('_camera_pose_prior_trans_scale', trans_scale)
        self.has_camera_pose_prior_target = bool(has_pose_target)

        has_intr_target = (
            self.camera_intrinsics is not None
            and isinstance(self.camera_intrinsics, (list, tuple))
            and len(self.camera_intrinsics) == self.n_imgs
        )
        if has_intr_target:
            intr_target = torch.stack([K.float() for K in self.camera_intrinsics], dim=0)
        else:
            intr_target = torch.zeros((0, 3, 3), dtype=torch.float32)
        self.register_buffer('_camera_intrinsics_target', intr_target)
        self.has_camera_intrinsics_prior_target = bool(has_intr_target)

    def _camera_pose_prior_loss(self, current_poses, target_poses):
        if current_poses.shape != target_poses.shape or current_poses.shape[0] == 0:
            return current_poses.new_zeros(())
        rel = torch.matmul(torch.linalg.inv(target_poses), current_poses)
        rot = rel[:, :3, :3]
        eye = torch.eye(3, device=rot.device, dtype=rot.dtype).unsqueeze(0)
        rot_loss = torch.norm(rot - eye, dim=(1, 2)).mean()
        trans = rel[:, :3, 3]
        trans_scale = torch.clamp(
            self._camera_pose_prior_trans_scale.to(device=trans.device, dtype=trans.dtype),
            min=1e-6,
        )
        trans_loss = (torch.norm(trans, dim=1) / trans_scale).mean()
        return rot_loss + self.camera_pose_prior_translation_weight * trans_loss

    def _camera_intrinsics_prior_loss(self, current_K, target_K, image_diag):
        if current_K.shape != target_K.shape or current_K.shape[0] == 0:
            return current_K.new_zeros(())
        focal_current = 0.5 * (current_K[:, 0, 0] + current_K[:, 1, 1])
        focal_target = 0.5 * (target_K[:, 0, 0] + target_K[:, 1, 1])
        focal_loss = torch.abs(
            torch.log(torch.clamp(focal_current, min=1e-6) / torch.clamp(focal_target, min=1e-6))
        ).mean()

        pp_current = current_K[:, :2, 2]
        pp_target = target_K[:, :2, 2]
    ...
    def forward_batchify(self, epoch=9999):
        ...
        if self.camera_pose_prior_weight > 0 and self.has_camera_pose_prior_target:
            pose_prior_loss = self._camera_pose_prior_loss(
                self.get_im_poses(),
                self._camera_pose_target.to(device=self.device),
            )
        else:
            pose_prior_loss = 0
        if self.camera_intrinsics_prior_weight > 0 and self.has_camera_intrinsics_prior_target:
            intrinsics_prior_loss = self._camera_intrinsics_prior_loss(
                self.get_intrinsics(),
                self._camera_intrinsics_target.to(device=self.device),
                self._camera_prior_image_diag,
            )
        else:
            intrinsics_prior_loss = 0
    ...
    def forward_window_wise(self, epoch=9999):
        ...
        win_start = int(getattr(self, 'win_start', self._get_window_bounds(self.window_idx)[0]))
        win_end = int(getattr(self, 'win_end', self._get_window_bounds(self.window_idx)[2]))
        if self.camera_pose_prior_weight > 0 and self.has_camera_pose_prior_target:
            pose_prior_loss = self._camera_pose_prior_loss(
                self.get_win_im_poses(),
                self._camera_pose_target.to(device=self.device)[win_start:win_end],
            )
        else:
            pose_prior_loss = 0
        if self.camera_intrinsics_prior_weight > 0 and self.has_camera_intrinsics_prior_target:
            intrinsics_prior_loss = self._camera_intrinsics_prior_loss(
                self.get_win_intrinsics(),
                self._camera_intrinsics_target.to(device=self.device)[win_start:win_end],
                self._camera_prior_image_diag[win_start:win_end],
            )
        else:
            intrinsics_prior_loss = 0
    ...
    def forward_non_batchify(self, epoch=9999):
        ...
        if self.camera_pose_prior_weight > 0 and self.has_camera_pose_prior_target:
            ...
            loss += self.camera_pose_prior_weight * pose_prior_loss
        if self.camera_intrinsics_prior_weight > 0 and self.has_camera_intrinsics_prior_target:
            intrinsics_prior_loss = self._camera_intrinsics_prior_loss(
                self.get_intrinsics(),
                self._camera_intrinsics_target.to(device=self.device),
            ...
            loss += self.camera_intrinsics_prior_weight * intrinsics_prior_loss
